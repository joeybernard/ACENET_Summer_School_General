...
int main ( );
unsigned char *julia_set ( int w, int h, int th, float xl, float xr, float yb, float yt );
...
int main ( int argc, char* argv[] )
{
  if ( argc < 4 )
    {
      printf ( "Usage: ./julia_set_openmp height width threads\n" );
      return 0;
    }
 int h = atoi(argv[1]);
 int w = atoi(argv[2]);
...
/*
  Purpose:
    ...
    MAIN is the main program for JULIA_OPENMP.
